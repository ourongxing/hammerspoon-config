...
local metadataKeys = {}
for k, v in fnutils.sortByKeys(obj) do
...
local neverSleepFunction = function(c, m, i, x, y)
	...
	if _internals.neverSleepCorner == "*" then
		return
	end
	...
	if m == "mouseEnter" then
		...
		if next(_internals.neverSleepLockModifiers) then
			...
			for k, v in pairs(currentMods) do
				if not fnutils.contains(validModifierKeys, k) then
					currentMods[k] = nil
				end
			end
...
local sleepNowFunction = function(c, m, i, x, y)
	-- this corner is disabled so skip
	if _internals.sleepNowCorner == "*" then
		return
	end
	-- if we're being displayed for reference, skip the triggering
	if _internals._showTimer then
	...
	if m == "mouseEnter" then
		...
		if doImmediateSleep then
			if _internals.immediateSleepShouldLock then
				...
				_internals._swallowModifierKeys = eventtap
						.new({ eventtap.event.types.flagsChanged }, function(ev)
							return true
						end)
						:start()
	...
	elseif m == "mouseExit" then
		...
		if _internals._clearModifierSwallow then
			_internals._clearModifierSwallow:fire()
		end
...
_neverSleepCanvas
		:mouseCallback(neverSleepFunction)
...
_neverSleepCanvas[#_neverSleepCanvas + 1] = {
	...
	image = image.imageFromASCII(neverSleepSymbol, {
		{ fillColor = { white = 1, alpha = 0.5 }, strokeColor = { alpha = 1 } },
		{ fillColor = { alpha = 0 },              strokeColor = { alpha = 1 } },
	}),
}

_sleepNowCanvas
		:mouseCallback(sleepNowFunction)
		:behavior("canJoinAllSpaces")
...
local positionCanvas = function(c, p)
	local frame = (type(_internals.sleepScreen) ~= "userdata" and _internals.sleepScreen() or _internals.sleepScreen)
			:fullFrame()
	if p == "UL" then
		c:frame({
			x = frame.x,
...
return setmetatable(obj, {
	...
	__tostring = function(self)
		...
		for i, v in ipairs(metadataKeys) do
			fieldSize = math.max(fieldSize, #v)
		end
		for i, v in ipairs(metadataKeys) do
	...
	__newindex = function(self, key, value)
		...
		elseif key == "sleepScreen" then
			local testValue = (
						type(value) == "function" or (type(value) == "table" and (getmetatable(value) or {}).__call)
					)
					and value()
					or value
		...
		elseif key == "immediateSleepModifiers" then
			...
			if type(value) == "table" then
				...
				if not isBad then
					_internals.immediateSleepModifiers = value
				end
			...
			if isBad then
				errMsg = errMsg
						.. "immediateSleepModifiers must be a table containg true or false values for one or more of the following keys: "
						.. table.concat(validModifierKeys, ", ")
			end
		elseif key == "neverSleepLockModifiers" then
			...
			if type(value) == "table" then
				...
				if not isBad then
					_internals.neverSleepLockModifiers = value
				end
			elseif type(value) == "nil" then
			...
			if isBad then
				errMsg = errMsg
						.. "neverSleepLockModifiers must be a table containg true or false values for one or more of the following keys: "
						.. table.concat(validModifierKeys, ", ")
			end
